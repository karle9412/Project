...
<div class="oooo" style="max-width:1800px; margin:auto;">
  ...
  <div id="main">
    <div class="pickup"  style="padding: 10px 20px;">픽업 할게요</div>
  </div>
  <div class="w3-container">
    ...
    <tr>
       ...
       <td width="30%" style="text-align:left"><a href="/Board/riderDetail?board_number=${board.board_number}&menu_id=${board.menu_id}&pageNum=1&contentNum=10">${board.title}</a></td>
       <td width="10%" style="text-align:center">${board.writer}</td>
       <td width="10%" style="text-align:center">${board.indate}</td>
       <td width="5%" style="text-align:center">${board.rider_local}</td>
    ...
    <tr>
    <td colspan="6">
      <c:set var="url" value="http://localhost:8080/Board/riderList?menu_id=MENU_01&pageNum=1&contentNum=10"/>
      <c:choose>
        ...
        <c:otherwise>
          <div id="BoardPa" class="BoardPa">
            <c:if test="${boardPager.prev}">
              <a class="page-linkA" href="/Board/riderList?menu_id=MENU_01&pageNum=${boardPager.getStartPage()-1}&contentNum=${(boardPager.getStartPage()-1)*10}&searchType=${map.searchType}&keyword=${map.keyword}" class="w3-button">< 이전</a>
            </c:if>
            <c:forEach begin="${boardPager.getStartPage()}" end="${boardPager.getEndPage()}" var="idx">
              <a class="page-linkA" href="/Board/riderList?menu_id=MENU_01&pageNum=${idx}&contentNum=${idx*10}&searchType=${map.searchType}&keyword=${map.keyword}" class="w3-button">${idx}</a>
            </c:forEach>
            <c:if test="${boardPager.next}">
              <a class="page-linkA" href="/Board/riderList?menu_id=MENU_01&pageNum=${boardPager.getEndPage()+1}&contentNum=${(boardPager.getEndPage()+1)*10}&searchType=${map.searchType}&keyword=${map.keyword}" class="w3-button">다음 ></a>
            </c:if>
          </div>
        </c:otherwise>
      </c:choose>
  ...
  <table class="ta">
    <tr style="border-top: 1px solid #000">
      ...
      <a href="/Board/RBoardWriteForm?menu_id=MENU_02&pageNum=${boardPager.getEndPage()}&contentNum=${(boardPager.getEndPage())*10}"
